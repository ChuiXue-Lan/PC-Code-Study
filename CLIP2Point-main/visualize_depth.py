...
def visualize_multi_view_depth(depth_map, save_path_base):
    """支持[2, H, W]或[2, 3, H, W]的npy文件，每个视角分别保存PNG"""
    if len(depth_map.shape) == 3:  # [2, H, W] 或 [C, H, W]
        if depth_map.shape[0] == 2:
            for i in range(2):
                save_path = save_path_base.replace('.png', f'_view{i+1}.png')
                visualize_depth(depth_map[i], save_path)
            return True
        else:
            # 兼容原有单视角[3, H, W]或[H, W]
            return visualize_depth(depth_map, save_path_base)
    elif len(depth_map.shape) == 4 and depth_map.shape[0] == 2:
        # [2, 3, H, W]
        for i in range(2):
            save_path = save_path_base.replace('.png', f'_view{i+1}.png')
            visualize_depth(depth_map[i], save_path)
...
def process_category(category_path, output_path, partition):
    ...
    for npy_file in tqdm(npy_files, desc=f"Visualizing {category_name}-{partition}"):
        ...
        if not visualize_multi_view_depth(depth_map, png_path):
