...
def offread_uniformed(filename, sampled_pt_num):
    """读取OFF文件并进行均匀采样
    
    Args:
        filename: OFF文件路径
        sampled_pt_num: 采样点数量
    
    Returns:
        numpy array: 采样后的点云数据
    """
    try:
        input = o3d.io.read_triangle_mesh(filename)
        
        # 检查mesh是否有效
        if len(input.triangles) == 0:
            print(f"Warning: Mesh has no triangles: {filename}")
            # 如果mesh无效，直接从顶点采样
            points = np.asarray(input.vertices)
            if len(points) >= sampled_pt_num:
                indices = np.random.choice(len(points), sampled_pt_num, replace=False)
                return points[indices]
            else:
                indices = np.random.choice(len(points), sampled_pt_num, replace=True)
                return points[indices]
        ...
        print(f"Error reading mesh file {filename}: {str(e)}")
        # 返回一个随机生成的点云作为备选
        return np.random.rand(sampled_pt_num, 3)
