...
class DepthFeatureEncoderV2(nn.Module):
    ...
    def __init__(self, encoder: UResnetEncoderV2, proj_dim=512, use_multi_scale=True):
        ...
        if not use_multi_scale:
            # 单尺度：双池化拼接后投影
            self.single_norm = nn.LayerNorm(encoder.out_channels * 2)  # 2C
            ...
            self.scale_norms = nn.ModuleList()  # 每尺度 LN(2C)
            total_proj_dim = 0
            for i in range(self.scales):
                scale_channels = encoder.ngf * (2 ** i)
                # 每个尺度双池化后投影到固定子维度（如64维）
                self.scale_norms.append(nn.LayerNorm(scale_channels * 2))     # 2C_i
                self.scale_projs.append(nn.Linear(scale_channels * 2, 64))    # 2C_i -> 64
                total_proj_dim += 64
            # 最终统一投影到目标维度
            self.final_norm = nn.LayerNorm(total_proj_dim)  # concat 后 LN
