...
class PointcloudRotate(object):
    ...
    def __call__(self, points, angle=None, flip=False):
        if np.random.uniform(0, 1) > self.p and angle is None and not flip:
            return points

        # 旋转角度：使用传入的 angle（单位：度），否则内部随机采样
        ...
        if self.axis is None:
            # 任意三轴随机旋转（用于更复杂的扰动）
            angles = np.random.uniform(size=3) * 2 * np.pi if angle is None else [rotation_angle]*3
        ...
        if not normals:
            rotated = torch.matmul(points, rotation_matrix.t())
        else:
            pc_xyz = points[:, 0:3]
            pc_normals = points[:, 3:]
            rotated_xyz = torch.matmul(pc_xyz, rotation_matrix.t())
            rotated_normals = torch.matmul(pc_normals, rotation_matrix.t())
            rotated = torch.cat([rotated_xyz, rotated_normals], dim=1)

        # 应用翻转（flip = True 则水平翻转 x 轴）
        if flip:
            rotated[:, 0] *= -1  # 仅对 x 轴翻转
...
class PointcloudUpSampling(object):
    ...
    def __call__(self, points):
        ...
        # except:
        #     print(adj_topk.shape, (uniform > median).shape)
        #     choice = adj_topk[uniform > median]
        choice = adj_topk
