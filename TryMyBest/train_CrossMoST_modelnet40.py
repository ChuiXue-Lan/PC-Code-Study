...
def get_args():
    ...
    parser.add_argument('--load_checkpoint', type=str, default='',
                        help='path to specific checkpoint to load and continue training')
    # Data
    ...
    parser.add_argument('--pretrain_dataset_name', default='modelnet40_img_pcl', type=str)
    ...
    # store_true的意思是：如果在命令行中加上--amp参数，则amp为True；如果没有加--amp，则amp为False。
    ...
    parser.add_argument('--slip_model', type=str,
                        default='/home/amaya/repos/CLIP-goes-3D/models/SLIP/slip_base_100ep.pt',
    ...
    parser.add_argument('--test_ckpt_addr',
                        default='/home/amaya/repos/ULIP/pretrained_weights/pretrained_models_ckpt_zero-sho_classification_checkpoint_pointbert.pt',
...
def main(args):
    ...
    if True:  # args.distributed:
        # 分布式训练设置
        num_tasks = utils.get_world_size()  # 获取总进程数
        global_rank = utils.get_rank()      # 获取当前进程的rank
        # 创建分布式采样器,用于训练数据
    ...
    # 创建训练数据加载器
    # 在 train_one_epoch 函数中，(images_weak, images_strong, mask, pc_weak, pc_strong, pc_mask), targets 是从 data_loader_train 中获取的一个 batch。
    # 这里 data_loader_train 的每个 batch 由 dataset_train 决定，dataset_train 的 __getitem__ 方法需要返回如下结构：
    #   ((images_weak, images_strong, mask, pc_weak, pc_strong, pc_mask), targets)
    # 也就是说，dataset_train 是一个自定义的数据集类，其 __getitem__ 方法会对每个样本进行多模态增强（如图像和点云的弱/强增强、掩码生成等），
    # 并将这些增强后的数据和标签一起返回。DataLoader 负责批量化这些数据，最终每个 batch 就是上述结构。
    data_loader_train = torch.utils.data.DataLoader(
        dataset_train,                                # 训练数据集，需实现 __getitem__ 返回 ((images_weak, images_strong, mask, pc_weak, pc_strong, pc_mask), targets)
        batch_size=args.batch_size,                   # 每个批次的样本数
        shuffle=(sampler_train is None),              # 当没有采样器时随机打乱数据
        num_workers=args.workers,                     # 数据加载的工作进程数
        pin_memory=True,                              # 将数据放入固定内存中以加快GPU传输
        sampler=sampler_train,                        # 数据采样器,用于分布式训练
    ...
    # 创建优化器
    # 根据assigner是否为None来决定是否使用层级学习率衰减
    optimizer = create_optimizer(
        args, model_without_ddp,
        get_num_layer=assigner.get_layer_id if assigner is not None else None,  # 获取层号的函数
        get_layer_scale=assigner.get_scale if assigner is not None else None)   # 获取层级缩放系数的函数
    
    # 设置混合精度训练
    if args.amp:  # 如果使用自动混合精度
        loss_scaler = NativeScaler()  # 创建梯度缩放器
        amp_autocast = torch.cuda.amp.autocast  # 使用CUDA的自动混合精度
    else:
        loss_scaler = None
        amp_autocast = suppress  # 不使用混合精度时,使用空操作

    # 创建余弦学习率调度器
    # 在训练过程中,学习率从args.lr逐渐衰减到args.min_lr
    # 支持预热(warmup)阶段,可以设置预热的epoch数和steps数
    lr_schedule_values = utils.cosine_scheduler(
        args.lr, args.min_lr, args.epochs, num_training_steps_per_epoch,
        warmup_epochs=args.warmup_epochs, warmup_steps=args.warmup_steps,
    )

    # 如果指定了load_checkpoint，优先加载指定的checkpoint
    if args.load_checkpoint:
        ...
        is_model_multi_gpus = True if list(model.state_dict())[0].split('.')[0] == 'module' else False
        is_ckpt_multi_gpus = True if list(state_dict)[0].split('.')[0] == 'module' else False
        
        for k, v in state_dict.items():
            if is_model_multi_gpus and not is_ckpt_multi_gpus:
                # 需要添加"module."前缀
                k = 'module.' + k
            elif not is_model_multi_gpus and is_ckpt_multi_gpus:
                # 需要删除"module."前缀
                k = k.replace('module.', '')
            new_state_dict[k] = v
        ...
        print(f"Loaded checkpoint successfully. Continuing from epoch {args.start_epoch}")
    else:
        ...
        if args.output_dir and utils.is_main_process() and (epoch + 1) % args.eval_freq == 0:
            ...
            if max_accuracy < test_stats["acc1_pc"]:
                ...
                if args.output_dir:
                    ...
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch=epoch, is_best=True, model_ema=model_ema)
                    print(f"========================= save best model at epoch:%d ========================="%epoch)
