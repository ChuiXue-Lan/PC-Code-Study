...
def cosine_scheduler(base_value, final_value, epochs, niter_per_ep, warmup_epochs=0,
    ...
    """
    该函数用于生成余弦退火学习率调度表（cosine annealing scheduler），常用于深度学习训练过程中动态调整学习率。

    参数说明：
    - base_value: 预热结束后的初始学习率（通常为主学习率）。
    - final_value: 训练结束时的最小学习率。
    - epochs: 总训练轮数。
    - niter_per_ep: 每个epoch的迭代次数（即每个epoch有多少个batch）。
    - warmup_epochs: 预热阶段的epoch数（可选，默认0）。
    - start_warmup_value: 预热开始时的学习率（可选，默认0）。
    - warmup_steps: 预热阶段的总步数（可选，默认-1，表示不用步数而用epoch数）。

    返回值：
    - schedule: 一个长度为 epochs * niter_per_ep 的numpy数组，表示每一步的学习率。

    主要流程：
    1. 计算预热阶段的总步数（warmup_iters），优先使用warmup_steps，否则用warmup_epochs * niter_per_ep。
    2. 如果有预热阶段，则用np.linspace生成从start_warmup_value到base_value的线性预热学习率序列。
    3. 余下的训练步数，采用余弦退火公式生成学习率序列。
    4. 将预热和余弦退火阶段的学习率拼接起来，得到完整的调度表。
    5. 最后断言长度正确并返回。

    余弦退火公式：
    lr = final_value + 0.5 * (base_value - final_value) * (1 + cos(pi * i / T))
    其中i为当前步数，T为总步数。
...
def save_model(args, epoch, model, model_without_ddp, optimizer,is_best=False, loss_scaler=None, model_ema=None):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)
...
def create_optimizer(args, model):
    ...
    Returns:
        optimizer: 优化器实例
    """
    opt_lower = args.opt.lower() if hasattr(args, 'opt') else 'adamw'
    optimizer = None
    ...
    if opt_lower == 'adamw':
        ...
        raise ValueError(f"优化器 {opt_lower} 不支持")
