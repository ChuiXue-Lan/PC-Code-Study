...
class ModelNet40_img_pcl(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load depth image
        depth = pil_loader(self.depthpaths[index] + '_' + picked + '.png')
        ...
        if self.partition == 'train':
            ...
            if self.transform is not None:
                img, depth, point1 = self.transform(img, depth, point1)
            
            return (img, depth, point1), label
...
@DATASETS.register_module()
class ModelNet40_img_pcl_depth(data.Dataset):
    '''
    ...
    def __getitem__(self, index):
        ...
        picked = '0' * (3 - len(str(picked))) + str(picked)
        img = pil_loader(self.imgpaths[index] + '_' + picked + '.png')
        depth = np.load(self.depthpaths[index] + '_' + picked + '.npy')  # 加载.npy格式的深度图
        depth = torch.from_numpy(depth).unsqueeze(0)  # 添加通道维度 [H, W] -> [1, H, W]
...
@DATASETS.register_module()
class ModelNet40_img_pcl_depth_multiview(data.Dataset):
    def __init__(self, config):
        self.root = config.DATA_PATH
        self.subset = config.subset
        self.npoints = config.npoints
        ...
        self.use_height = False
        self.few = False
        if hasattr(config, 'few'):
            self.few = config.few
            self.few_num = config.few_num
            self.few_seed = config.few_seed
        self._load_data()
        if self.few:
            self._few()
    ...
    def _load_data(self):
        ...
        for line in lines:
            ...
            self.paths.append(os.path.join(self.root, 'pcl', taxonomy_id, model_id + '.txt'))
            self.labels.append(cats[taxonomy_id])
            
            # Add multiple views
            img_paths_per_model = []
            depth_paths_per_model = []
            for view_id in range(self.num_views):
                img_paths_per_model.append(os.path.join(self.root, 'img', taxonomy_id, model_id + f'_{view_id}.png'))
                depth_paths_per_model.append(os.path.join(self.root, 'depth', taxonomy_id, model_id + f'_{view_id}.npy'))
            self.img_paths.append(img_paths_per_model)
            self.depth_paths.append(depth_paths_per_model)

    def _few(self):
        random.seed(self.few_seed)
        few_paths = []
        few_labels = []
        few_img_paths = []
        few_depth_paths = []
        for i in range(self.num_classes):
            index = [j for j, x in enumerate(self.labels) if x == i]
            select_index = random.sample(index, self.few_num)
            for j in select_index:
                few_paths.append(self.paths[j])
                few_labels.append(self.labels[j])
                few_img_paths.append(self.img_paths[j])
                few_depth_paths.append(self.depth_paths[j])
        self.paths = few_paths
        self.labels = few_labels
        self.img_paths = few_img_paths
        self.depth_paths = few_depth_paths
    ...
    def __getitem__(self, index):
        ...
        # Load multiple views of images and depths
        imgs = []
        depths = []
        for view_id in range(self.num_views):
            # Load and process image
            img_path = self.img_paths[index][view_id]
            img = pil_loader(img_path)
            if self.subset == 'train':
                img = build_transform(True, None)(img)
            else:
                img = build_transform(False, None)(img)
            imgs.append(img)
            
            # Load and process depth
            depth_path = self.depth_paths[index][view_id]
            depth = np.load(depth_path)
            depth = torch.from_numpy(depth).float()
            depths.append(depth)
        
        imgs = torch.stack(imgs, dim=0)  # [num_views, C, H, W]
