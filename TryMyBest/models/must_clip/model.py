...
from torch.nn.init import constant_, xavier_normal_, xavier_uniform_, trunc_normal_
...
class VisionTransformer_MIM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask=None):
        ...
        if mask is not None:     
            ...
            loss_mim = F.l1_loss(x_rec, input, reduction='none')
            loss_mim = (loss_mim * mask).sum() / mask.sum() / x.size(1)

            return features, loss_mim, x_patch #loss_align
...
class DepthTransformer_MIM(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, mask: bool):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
    ...
    def forward(self, x: torch.Tensor, Mask=None):
        ...
        if self.mask and Mask is not None:
            B, L, D = x.shape
            if Mask is None:
                Mask = self.random_masking(B, L-1)
            Mask = torch.cat([torch.zeros(B, 1).bool().cuda(), Mask], dim=1)
            mask_tokens = self.mask_token.expand(B, L, -1)
            w = ~Mask
            w = w.unsqueeze(-1).type_as(mask_tokens)
            x = x * w + mask_tokens * (1 - w)
        ...
        if self.proj is not None:
            x = x[torch.arange(x.shape[0]), 0] @ self.proj

        return x

    def random_masking(self, B, L):
        len_keep = int(L * (1 - 0.4))
        noise = torch.rand(B, L, device='cuda')  # noise in [0, 1]
