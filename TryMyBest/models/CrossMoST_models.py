...
class CrossMoST_multiview(nn.Module):
    ...
    def forward(self, image, pc, text=None, Mask=None, val=False):
        ...
        if self.ulip:
            ...
            # image_logits, image_mim, x_patch_image = 100 * image_features @ self.classifier, image_embed[1], image_embed[2]
...
class CrossMoST(nn.Module):
    def __init__(self, args, config, point_encoder, **kwargs):
        ...
        self.image_projection = nn.Parameter(torch.empty(kwargs.vision_width, kwargs.embed_dim))
        self.depth_projection = nn.Parameter(torch.empty(kwargs.vision_width, kwargs.embed_dim))
        self.text_projection = nn.Parameter(torch.empty(kwargs.transformer_width, kwargs.embed_dim))
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
    ...
    def initialize_parameters(self):
        ...
        nn.init.normal_(self.image_projection, std=self.vision_width ** -0.5)
        nn.init.normal_(self.depth_projection, std=self.vision_width ** -0.5)
        nn.init.normal_(self.text_projection, std=self.transformer.width ** -0.5)
    ...
    def forward(self, image, depth, pc, text=None, Mask=None, val=False):
        pc_embed = self.encode_pc(pc, Mask)
        image_embed = self.encode_image(image, Mask)
        ...
        if self.ulip:
            if val:
                pc_embed = pc_embed @ self.pc_projection
                image_embed = image_embed @ self.image_projection
                depth_embed = depth_embed @ self.depth_projection
...
def CrossMoST_PointBERT(args, classes, templates, tokenizer):
    """
    构建支持三模态(RGB图像、深度图、点云)的CrossMoST模型。
    ...
    # 构建深度图编码器
    depth_model = DepthTransformer_MIM(
        input_resolution=image_resolution,
        patch_size=vision_patch_size,
        width=vision_width,
        layers=vision_layers,
    ...
    other_needed = [
        "point_encoder.encoder.first_conv.1.running_mean",
        "point_encoder.encoder.first_conv.1.running_var",
    ...
    temp['depth_projection'] = clip_model['visual.proj']  # 初始化深度图投影层
    ...
    if args.ulip:
        for k, p in model.named_parameters():
            ...
            elif 'pc_projection' in k or 'image_projection' in k or 'depth_projection' in k or 'text_projection' in k:
                p.requires_grad = True
            else:
...
class CrossMoST_3Modal(nn.Module):
    def __init__(self, config):
        ...
        self.image_encoder = VisionTransformer(
            ...
            mask_ratio=config.mask_ratio if config.mask else 0.0
        )
        
        # 点云编码器
        self.point_encoder = Point_BERT_Encoder(config)
        
        # 深度图编码器
        depth_backbone = UResnetEncoder(
        ...
        self.depth_encoder = DepthFeatureEncoder(
            encoder=depth_backbone,
            proj_dim=config.vision_width,
            use_multi_scale=True,
            mask=config.mask
        )
        
        # 特征投影层
        self.image_proj = nn.Linear(config.vision_width, config.embed_dim)
        self.point_proj = nn.Linear(config.pc_feat_dims, config.embed_dim)
        self.depth_proj = nn.Linear(config.vision_width, config.embed_dim)
    ...
    def encode_image(self, image, image_mask=None):
        image_feats = self.image_encoder(image, mask=image_mask)
        image_feats = self.image_proj(image_feats)
        return image_feats
        
    def encode_point(self, point, point_mask=None):
        point_feats = self.point_encoder(point, mask=point_mask)
        point_feats = self.point_proj(point_feats)
        return point_feats
        
    def encode_depth(self, depth, depth_mask=None):
        depth_feats = self.depth_encoder(depth, mask=depth_mask)
        depth_feats = self.depth_proj(depth_feats)
        return depth_feats
        
    def forward(self, batch):
        # 提取特征
        image_feats = self.encode_image(batch['image'], batch.get('image_mask'))
        point_feats = self.encode_point(batch['point'], batch.get('point_mask'))
        depth_feats = self.encode_depth(batch['depth'], batch.get('depth_mask'))
        ...
        if self.training:
            # 计算三个模态之间的对比损失
            loss_img_point = self.compute_contrastive_loss(
                image_feats[:, 0], 
                point_feats[:, 0]
            )
            loss_img_depth = self.compute_contrastive_loss(
                image_feats[:, 0], 
                depth_feats[:, 0]
            )
            loss_point_depth = self.compute_contrastive_loss(
                point_feats[:, 0], 
                depth_feats[:, 0]
            )
            ...
            # 计算掩码重建损失
            loss_mim = 0
            if batch.get('image_mask') is not None:
                loss_mim += self.compute_mim_loss(
                    image_feats, 
                    batch['image'], 
                    batch['image_mask']
                )
            if batch.get('point_mask') is not None:
                loss_mim += self.compute_mim_loss(
                    point_feats, 
                    batch['point'], 
                    batch['point_mask']
                )
            if batch.get('depth_mask') is not None:
                loss_mim += self.compute_mim_loss(
                    depth_feats, 
                    batch['depth'], 
                    batch['depth_mask']
            ...
            return {
                'logits': logits,
                'loss_img_point': loss_img_point,
                'loss_img_depth': loss_img_depth,
                'loss_point_depth': loss_point_depth,
                'loss_mim': loss_mim
            }
            
        return {'logits': logits}
        
    def compute_contrastive_loss(self, feat1, feat2, temperature=0.07):
    ...
    def compute_mim_loss(self, pred_feats, target, mask):
        # 根据具体的掩码预测任务实现
        # 这里需要根据不同模态的特点来实现具体的重建损失
        pass
...
class CrossModalFusionLayer(nn.Module):
    def __init__(self, dim, num_heads=8, mlp_ratio=4., qkv_bias=False,
        ...
        self.attn = nn.MultiheadAttention(dim * 3, num_heads, dropout=attn_drop)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim * 3, hidden_features=mlp_hidden_dim, out_features=dim)
        
    def forward(self, x_img, x_point, x_depth):
        # 拼接三个模态的特征
        x = torch.cat([x_img, x_point, x_depth], dim=-1)
        x = self.norm1(x)
