*** Begin Patch
*** Update File: Render/obj2off_batch.py
@@
-import os
-import subprocess
...
-RENDER_SCRIPT = os.path.abspath('D:\Pycharm\Replicate\Render\obj2off.py')
-VIEWS = 30
-RESOLUTION = 224
-
...
+# 配置
+SHAPENETCORE_ROOT = 'F:/Datasets/ShapeNet/ShapeNetCore/ShapeNetCore.v1.withoutimages'
+RENDER_SCRIPT = os.path.abspath(r'D:\Pycharm\Replicate\Render\obj2off.py')
+
+
+def collect_models(root_dir):
+    items = []
+    for cat in os.listdir(root_dir):
+        cat_dir = os.path.join(root_dir, cat, cat)
+        if not os.path.isdir(cat_dir):
+            continue
+        for model_id in os.listdir(cat_dir):
...
+            if os.path.isfile(obj_path):
+                items.append((cat, model_id, obj_path))
+    return items
+
+
...
+    print(f'[{idx+1}] 转换: {cat}-{model_id}')
+    devnull = 'nul' if os.name == 'nt' else '/dev/null'
+    try:
+        with open(devnull, 'w') as f:
+            subprocess.run(cmd, stdout=f, stderr=subprocess.STDOUT, check=True)
+    except subprocess.CalledProcessError as e:
+        print(f'转换失败: {cat}-{model_id}, 错误: {e}')
+
+
+if __name__ == '__main__':
+    models = collect_models(SHAPENETCORE_ROOT)
+    print(f'共找到 {len(models)} 个模型')
+
+    with Pool(cpu_count()) as pool:
+        pool.map(convert_one, list(enumerate(models)))
*** End Patch