...
from models.TMB.UResnetEncoder import UResnetEncoder
from models.TMB.DepthFeatureEncoder import DepthFeatureEncoder
...
class ULIP_WITH_IMAGE(nn.Module):
    def __init__(self, point_encoder, depth_encoder=None, **kwargs):
        ...
        self.depth_projection = nn.Parameter(torch.empty(kwargs.depth_feat_dims, kwargs.embed_dim))
    ...
    def initialize_parameters(self):
        # 初始化token_embedding的权重，标准差为0.02
        nn.init.normal_(self.token_embedding.weight, std=0.02)
        # 初始化位置编码参数，标准差为0.01
        nn.init.normal_(self.positional_embedding, std=0.01)

        # 计算transformer中各层参数初始化的标准差
        proj_std = (self.transformer.width ** -0.5) * ((2 * self.transformer.layers) ** -0.5)
        attn_std = self.transformer.width ** -0.5
        fc_std = (2 * self.transformer.width) ** -0.5

        # 遍历transformer的每个残差块，初始化注意力和MLP相关的权重
        ...
        # 初始化图像投影层的权重，标准差与vision_width相关
        nn.init.normal_(self.image_projection, std=self.vision_width ** -0.5)
        # 初始化文本投影层的权重，标准差与transformer宽度相关
    ...
    def encode_depth(self, depth, Mask=None):
        # print("encode_depth_mask")
        if self.depth_encoder is None:
            # 如果没有深度编码器，返回零张量
            batch_size = depth.size(0)
            return torch.zeros(batch_size, self.depth_projection.size(1), device=depth.device)
        # depth_feat = self.depth_encoder(depth, Mask)
        depth_feat = self.depth_encoder(depth)
        # print(f"depth_feat shape: {depth_feat.shape}, depth_projection shape: {self.depth_projection.shape}")
        depth_embed = depth_feat @ self.depth_projection
        # print(f"depth_embed shape: {depth_embed.shape}, depth_embed norm: {depth_embed.norm()}")
        return depth_embed

    def forward(self, pc, text, image=None, depth=None):
        ...
        depth_embed = self.encode_depth(depth)
...
def get_metric_names(model):
    ...
    return ['loss', 'ulip_loss', 'ulip_pc_image_acc', 'ulip_pc_text_acc', 'ulip_pc_depth_acc']
...
def ULIP_PN_SSG(args):
    ...
    from models.TMB.DepthFeatureEncoder import DepthFeatureEncoder
    from models.TMB.UResnetEncoder import UResnetEncoder
    depth_encoder_backbone = UResnetEncoder(ngf=32, n_blocks=1, n_down=2)
    depth_encoder = DepthFeatureEncoder(depth_encoder_backbone, proj_dim=512, use_multi_scale=True)
    depth_feat_dims = 512
    # =====================================================================

    model = ULIP_WITH_IMAGE(embed_dim=512, vision_width=768, point_encoder=point_encoder, depth_encoder=depth_encoder, vision_model=vision_model,
                            context_length=77, vocab_size=49408,
                            transformer_width=512, transformer_heads=8, transformer_layers=12, pc_feat_dims=pc_feat_dims, depth_feat_dims=depth_feat_dims)
