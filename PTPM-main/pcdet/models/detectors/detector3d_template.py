...
class Detector3DTemplate(nn.Module):
    ...
    def load_params_from_file(self, filename, logger, to_cpu=False, pre_trained_path=None):
        if 's3://' in str(filename):
            if not self.petrel_client.isfile(filename):
        ...
        if 's3://' in str(filename):
            checkpoint = self.petrel_client.load_ckpt(filename, map_location=loc_type)
        else:
            checkpoint = torch.load(filename, map_location=loc_type)

        model_state_disk = checkpoint['model_state']
        if not pre_trained_path is None:
            if 's3://' in str(pre_trained_path):
                pretrain_checkpoint = self.petrel_client.load_ckpt(pre_trained_path, map_location=loc_type)
            else:
            ...
            if 'optimizer_state' in checkpoint and checkpoint['optimizer_state'] is not None:
                ...
                if os.path.exists(optimizer_filename):
                    if 's3://' in str(optimizer_filename):
                        optimizer_ckpt = self.petrel_client.load_ckpt(optimizer_filename, map_location=loc_type)
                    else:
