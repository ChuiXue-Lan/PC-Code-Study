...
class WaymoDataset(DatasetTemplate):
    ...
    def get_pseudo_infos(self, info_path, result_path=None, sampled_interval=1, use_ceph=False):
        print('---------------The waymo sample interval is %d, total sequecnes is %d-----------------'
              % (sampled_interval, len(self.sample_sequence_list)))
        
        cur_seq_info = []
        if 's3://' in str(result_path):
            from ..ceph import PetrelBackend
            self.petrel_client = PetrelBackend()
            infer_result = self.petrel_client.load_pkl(str(result_path), 1)[0]
        else:
            infer_result = pickle.load(open(result_path, 'rb'))

        for idx in tqdm(range(len(self.infos))):
            self.infos[idx]['annos'].pop('difficulty')
            self.infos[idx]['annos'].pop('obj_ids')
            self.infos[idx]['annos'].pop('tracking_difficulty')
            self.infos[idx]['annos'].pop('num_points_in_gt')
            self.infos[idx]['annos'].pop('speed_global')
            self.infos[idx]['annos'].pop('accel_global')
            self.infos[idx]['annos']['name'] = infer_result[idx]['name']
            self.infos[idx]['annos']['score'] = infer_result[idx]['score']
            self.infos[idx]['annos']['gt_boxes_lidar'] = infer_result[idx]['boxes_lidar'][:, :7]
            self.infos[idx]['annos']['location'] = infer_result[idx]['boxes_lidar'][:, :3]
            self.infos[idx]['annos']['dimensions'] = infer_result[idx]['boxes_lidar'][:, 3:6]
            self.infos[idx]['annos']['heading_angles'] = infer_result[idx]['boxes_lidar'][:, [6]].reshape(-1)
            assert self.infos[idx]['frame_id'] == infer_result[idx]['frame_id']

            cur_seq_info.append(self.infos[idx])
            if (idx == len(self.infos) - 1) or \
                (self.infos[idx]['point_cloud']['lidar_sequence'] != self.infos[idx + 1]['point_cloud']['lidar_sequence']):

                sequence_name = self.infos[idx]['point_cloud']['lidar_sequence']
                if use_ceph:
                    info_path_list = str(info_path).split('/')
                    info_path_ceph = os.path.join(os.path.join(*self.ceph_path.split('/')[:-1]), info_path_list[-2], self.dataset_cfg.PROCESSED_DATA_TAG, sequence_name, sequence_name + '.pkl').replace('s3:/', 's3://')
                    self.petrel_client.save_pkl(info_path_ceph, [cur_seq_info])
                    cur_seq_info = []
                else:
                    info_path_list = str(info_path).split('/')
                    cur_seq_info_path = os.path.join(info_path_list[:-1], self.dataset_cfg.PROCESSED_DATA_TAG, sequence_name, sequence_name + '.pkl')
                    with open(cur_seq_info_path, 'wb') as f:
                        pickle.dump(cur_seq_info, f)

        if idx == len(self.infos) - 1:
            info_path_list = str(info_path).split('/')
            info_path_ceph = os.path.join(os.path.join(*self.ceph_path.split('/')[:-1]), *info_path_list[-2:]).replace('s3:/', 's3://')
            if sampled_interval > 1:
                self.sampled_infos = []
                for k in range(0, len(self.infos), sampled_interval):
                    self.sampled_infos.append(self.infos[k])
                if use_ceph:
                    self.petrel_client.save_pkl(info_path_ceph, [self.sampled_infos])
                else:
                    with open(info_path, 'wb') as f:
                        pickle.dump(self.sampled_infos, f)
                return self.sampled_infos
            else:
                if use_ceph:
                    self.petrel_client.save_pkl(info_path_ceph, [self.infos])
                else:
                    with open(info_path, 'wb') as f:
    ...
    def create_groundtruth_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
                                    processed_data_tag=None, use_ceph=False, infos=None):
        if use_ceph:
            from ..ceph import PetrelBackend
            self.petrel_client = PetrelBackend()
        ...
        if infos is None:
            if use_ceph:
                info_path_list = str(info_path).split('/')
                info_path_ceph = os.path.join(self.ceph_path, info_path_list[-1])
        ...
        for k in tqdm(range(0, len(infos), sampled_interval)):
            ...
            assert gt_boxes.shape[0] == obj_ids.shape[0]
            ...
            for i in range(num_obj):
                ...
                if (used_classes is None) or names[i] in used_classes:
                    ...
                    if use_ceph:
                        ...
                        filepath_ceph = os.path.join(os.path.join(*self.ceph_path.split('/')[:-1]), filepath_list[-3], filepath_list[-2], filepath_list[-1]).replace('s3:/', 's3://')
                    ...
                    db_info = {'name': names[i], 'path': db_path, 'sequence_name': sequence_name,
                               'sample_idx': sample_idx, 'gt_idx': i, 'obj_id': obj_ids[i], 'box3d_lidar': gt_boxes[i],
        ...
        if use_ceph:
            ...
            db_info_save_path_ceph = os.path.join(os.path.join(*self.ceph_path.split('/')[:-1]), db_info_save_path_list[-2], db_info_save_path_list[-1]).replace('s3:/', 's3://')
        ...
        if use_ceph:
            db_data_save_path_list = str(db_data_save_path).split('/')
            db_data_save_path_ceph = os.path.join(os.path.join(*self.ceph_path.split('/')[:-1]), db_data_save_path_list[-2], db_data_save_path_list[-1]).replace('s3:/', 's3://')
            self.petrel_client.save_np(db_data_save_path_ceph, stacked_gt_points)
        else:
    ...
    def create_gt_database_of_single_scene(self, info_with_idx, database_save_path=None, use_sequence_data=False, used_classes=None,
                                           total_samples=0, use_cuda=False, use_ceph=False, crop_gt_with_tail=False):
    ...
    def create_groundtruth_database_parallel(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
                                             processed_data_tag=None, use_ceph=False, infos=None, num_workers=16, crop_gt_with_tail=False):
        if use_ceph:
            from ..ceph import PetrelBackend
            self.petrel_client = PetrelBackend()
        ...
        create_gt_database_of_single_scene = partial(
            ...
            crop_gt_with_tail=crop_gt_with_tail, use_ceph=use_ceph,
        )
    ...
    def create_pseudo_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
                                    processed_data_tag=None, use_ceph=False, infos=None, full_db=False):
        """
        创建伪标签数据库。主要功能包括:
        1. 从原始数据中提取伪标签框内的点云
        2. 将点云和对应的标注信息保存到数据库中
        3. 支持序列数据和单帧数据两种模式
        4. 支持ceph存储和本地存储两种方式
        ...
        - info_path: 数据信息路径
        - save_path: 保存路径 
        - used_classes: 需要使用的类别
        - split: 数据集划分(train/val等)
        - sampled_interval: 采样间隔
        - processed_data_tag: 处理后的数据标签
        - use_ceph: 是否使用ceph存储
        - infos: 数据信息列表
        - full_db: 是否使用全量数据库
        """
        ...
        if use_sequence_data:
            st_frame, ed_frame = self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[0], self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[1]
            self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[0] = min(-4, st_frame)  # 至少使用5帧用于生成gt数据库
            st_frame = self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[0]
            database_save_path = save_path / ('%s_gt_database_%s_sampled_%d_multiframe_%s_to_%s' % (processed_data_tag, split, sampled_interval, st_frame, ed_frame))
            db_info_save_path = save_path / ('%s_waymo_dbinfos_%s_sampled_%d_multiframe_%s_to_%s.pkl' % (processed_data_tag, split, sampled_interval, st_frame, ed_frame))
            db_data_save_path = save_path / ('%s_gt_database_%s_sampled_%d_multiframe_%s_to_%s_global.npy' % (processed_data_tag, split, sampled_interval, st_frame, ed_frame))
            ...
            if full_db is False:
                database_save_path = save_path / ('%s_pseudo_database_%s_sampled_%d' % (processed_data_tag, split, sampled_interval))
                db_info_save_path = save_path / ('%s_waymo_pseudo_dbinfos_%s_sampled_%d.pkl' % (processed_data_tag, split, sampled_interval))
            else:
                database_save_path = save_path / ('%s_pseudo_database_%s_sampled_full' % (processed_data_tag, split))
                db_info_save_path = save_path / ('%s_waymo_pseudo_dbinfos_%s_sampled_full.pkl' % (processed_data_tag, split))
        ...
        # 遍历数据集,提取伪标签框内的点云
        for k in tqdm(range(0, len(infos), sampled_interval)):
            print('pseudo_database sample: %d/%d' % (k + 1, len(infos)))
            info = infos[k]
            ...
            for i in range(num_obj):
                ...
                if (used_classes is None) or names[i] in used_classes:
                    ...
                    db_info = {'name': names[i], 'path': db_path, 'sequence_name': sequence_name,
                               'sample_idx': sample_idx, 'gt_idx': i, 'box3d_lidar': gt_boxes[i],
                               'num_points_in_gt': gt_points.shape[0], 'score': score[i]}
    ...
    def create_pseudo_database_of_single_scene(self, info_with_idx, database_save_path, 
                used_classes=None, total_samples=0, use_cuda=False, use_ceph=False, full_db=False):
        info, info_idx = info_with_idx
        print('gt_database sample: %d/%d' % (info_idx, total_samples))
    ...
    def create_pseudo_database_parallel(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
                                    processed_data_tag=None, use_ceph=False, infos=None, num_workers=16, full_db=False):
        if use_ceph:
            from ..ceph import PetrelBackend
            self.petrel_client = PetrelBackend()
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
                       ...
                       workers=min(16, multiprocessing.cpu_count()), update_info_only=False, use_ceph=False):
    ...
    if use_ceph:
        ...
        train_filename_list = str(train_filename).split('/')
        train_filename_ceph = os.path.join(os.path.join(*dataset.ceph_path.split('/')[:-1]), train_filename_list[-2], train_filename_list[-1])
        petrel_client.save_pkl(train_filename_ceph, [waymo_infos_train])
    else:
    ...
    if use_ceph:
        from ..ceph import PetrelBackend
        petrel_client = PetrelBackend()
        val_filename_list = str(val_filename).split('/')
        val_filename_ceph = os.path.join(os.path.join(*dataset.ceph_path.split('/')[:-1]), val_filename_list[-2], val_filename_list[-1])
        petrel_client.save_pkl(val_filename_ceph, [waymo_infos_val])
    else:
    ...
    dataset.create_groundtruth_database(
        ...
        used_classes=['Vehicle', 'Pedestrian', 'Cyclist'], processed_data_tag=processed_data_tag, use_ceph=use_ceph
    )
    print('---------------Data preparation Done---------------')

def create_waymo_infos_lpart(dataset_cfg, class_names, data_path, save_path,
                       raw_data_tag='raw_data', processed_data_tag='waymo_processed_data',
                       workers=min(16, multiprocessing.cpu_count()), update_info_only=False, use_ceph=False, use_parallel=True):
    dataset = WaymoDataset(
        dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path,
        training=False, logger=common_utils.create_logger()
    )
    ...
    print('---------------Start to load data infos---------------')
    dataset.set_split(train_split)
    ...
    if use_parallel:
        dataset.create_groundtruth_database_parallel(
            info_path=train_filename, save_path=save_path, split='train', sampled_interval=1,
            used_classes=['Vehicle', 'Pedestrian', 'Cyclist'], processed_data_tag=processed_data_tag, use_ceph=use_ceph, infos=dataset.infos,
...
def create_waymo_infos_upart(dataset_cfg, class_names, data_path, save_path,
                       raw_data_tag='raw_data', processed_data_tag='waymo_processed_data',
                       workers=min(16, multiprocessing.cpu_count()), update_info_only=False, 
                       use_ceph=False, result_path=None, ceph_path=None, use_parallel=True, full_db=False, ratio=0.05):
    dataset = WaymoDataset(
        dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path,
        training=False, logger=common_utils.create_logger()
    )
    ...
    if result_path != '':
        ...
        infos = dataset.get_pseudo_infos(
            ...
            sampled_interval=1 if ratio!='0.02' else 4,
            use_ceph=use_ceph
        )
    else:
        print('---------------Start to load data infos---------------')
        temp = copy.deepcopy(dataset.data_path_ceph)
        dataset.data_path_ceph = os.path.join(ceph_path, dataset.dataset_cfg.PROCESSED_DATA_TAG)
        dataset.set_split(train_split)
        dataset.data_path_ceph = temp
        infos = dataset.infos

    print('---------------Start create groundtruth database for data augmentation---------------')
    if use_parallel:
        dataset.create_pseudo_database_parallel(
            info_path=train_filename, save_path=save_path, split='train', sampled_interval=1,
            used_classes=['Vehicle', 'Pedestrian', 'Cyclist'], processed_data_tag=processed_data_tag, use_ceph=use_ceph, infos=infos, num_workers=workers, full_db=full_db
        )
    else:
        dataset.create_pseudo_database(
            info_path=train_filename, save_path=save_path, split='train', sampled_interval=1,
            used_classes=['Vehicle', 'Pedestrian', 'Cyclist'], processed_data_tag=processed_data_tag, use_ceph=use_ceph, infos=infos, full_db=full_db
        )
    print('---------------Data preparation Done---------------')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_parallel', action='store_true', default=False, help='')
    ...
    parser.add_argument('--ratio', type=str, default='0.05')
    parser.add_argument('--pseudo_label', action='store_true', default=False)
    parser.add_argument('--result_path', type=str, default='')
    parser.add_argument('--result_path_car', type=str, default='')
    parser.add_argument('--result_path_ped', type=str, default='')
    parser.add_argument('--result_path_cyc', type=str, default='')
    parser.add_argument('--post_fix', type=str, default='')
    parser.add_argument('--ceph_path', type=str, default='')
    parser.add_argument('--full_db', action='store_true', default=False)
    ...
    if args.func == 'create_waymo_infos':
    ...
    elif args.func == 'create_waymo_infos_lpart':
        try:
            yaml_config = yaml.safe_load(open(args.cfg_file), Loader=yaml.FullLoader)
        except:
            yaml_config = yaml.safe_load(open(args.cfg_file))
        dataset_cfg = EasyDict(yaml_config)
        dataset_cfg.PROCESSED_DATA_TAG = args.processed_data_tag
        create_waymo_infos_lpart(
            dataset_cfg=dataset_cfg,
            class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
            data_path=ROOT_DIR / 'data' / ('waymo_lpart_%s'%args.ratio),
            save_path=ROOT_DIR / 'data' / ('waymo_lpart_%s'%args.ratio),
            raw_data_tag='raw_data',
            processed_data_tag=args.processed_data_tag,
            update_info_only=args.update_info_only,
            use_ceph=args.use_ceph
        )
    elif args.func == 'create_waymo_infos_upart':
        try:
            yaml_config = yaml.safe_load(open(args.cfg_file), Loader=yaml.FullLoader)
        except:
            yaml_config = yaml.safe_load(open(args.cfg_file))
        postfix = args.func.split('create_waymo_infos_upart')[-1]
        dataset_cfg = EasyDict(yaml_config)
        dataset_cfg.PROCESSED_DATA_TAG = args.processed_data_tag
        create_waymo_infos_upart(
            dataset_cfg=dataset_cfg,
            class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
            data_path=ROOT_DIR / 'data' / ('waymo_upart_%s'%args.ratio) if args.post_fix=='' else ROOT_DIR / 'data' / ('waymo_upart_%s_%s'%(args.ratio, args.post_fix)),
            save_path=ROOT_DIR / 'data' / ('waymo_upart_%s'%args.ratio) if args.post_fix=='' else ROOT_DIR / 'data' / ('waymo_upart_%s_%s'%(args.ratio, args.post_fix)),
            raw_data_tag='raw_data',
            processed_data_tag=args.processed_data_tag,
            update_info_only=args.update_info_only,
            use_ceph=args.use_ceph,
