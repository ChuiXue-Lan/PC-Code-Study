...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.ceph_path = sampler_cfg.get('CEPH_PATH', None)
        if self.ceph_path:
        ...
        for db_info_path in sampler_cfg.DB_INFO_PATH:
            if self.ceph_path is None:
                ...
                db_info_path = os.path.join(self.ceph_path, db_info_path)
                infos = self.petrel_client.load_pkl(str(db_info_path), 1)[0]
                [self.db_infos[cur_class].extend(infos[cur_class]) for cur_class in class_names]
    ...
    def filter_by_min_scores(self, db_infos, min_scores_list):
        for name_num in min_scores_list:
            name, min_score = name_num.split(':')
            ...
            if name in db_infos.keys():
                ...
                if self.logger is not None:
                    self.logger.info('Database filter by min scores %s: %d => %d' %
                                     (name, len(db_infos[name]), len(filtered_infos)))
                db_infos[name] = filtered_infos

        return db_infos

    def filter_by_max_scores(self, db_infos, max_scores_list):
        for name_num in max_scores_list:
            name, max_score = name_num.split(':')
            ...
            if name in db_infos.keys():
                ...
                if self.logger is not None:
                    self.logger.info('Database filter by max scores %s: %d => %d' %
                                     (name, len(db_infos[name]), len(filtered_infos)))
                db_infos[name] = filtered_infos

        return db_infos

    def filter_by_distance(self, db_infos, distance_param):
        new_db_infos = {}
        for key, dinfos in db_infos.items():
            pre_len = len(dinfos)
            new_db_infos[key] = []
            for info in dinfos:
                box_dist = np.linalg.norm(info['box3d_lidar'][:2], ord=2, axis=0, keepdims=False)
                if (distance_param[0] == 'close') and (box_dist <= distance_param[1]):
                        new_db_infos[key].append(info)
                if (distance_param[0] == 'far') and (box_dist > distance_param[1]):
                        new_db_infos[key].append(info)

            if self.logger is not None:
                self.logger.info('Database filter by distance %s: %d => %d' % (key, pre_len, len(new_db_infos[key])))
        return new_db_infos
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        if 'gt_boxes_mask' in data_dict.keys():
            ...
            gt_score = data_dict['gt_score'][gt_boxes_mask]
        else:
            gt_boxes = data_dict['gt_boxes']
            gt_names = data_dict['gt_names']
            gt_score = data_dict['gt_score']
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                if self.ceph_path is None:
                    ...
                    file_path = os.path.join(self.ceph_path, info['path'])
                    obj_points = np.copy(self.petrel_client.load_bin(str(file_path), dtype='float32').reshape(
                        [-1, self.sampler_cfg.NUM_POINT_FEATURES]))
        ...
        sampled_gt_score = np.array([x['score'] for x in total_valid_sampled_dict])
        ...
        gt_score = np.concatenate([gt_score, sampled_gt_score], axis=0)
        data_dict['gt_boxes'] = gt_boxes
        data_dict['gt_names'] = gt_names
        data_dict['gt_score'] = gt_score
        # print(sampled_gt_score)
        data_dict['points'] = points

        if self.img_aug_type is not None:
            data_dict = self.copy_paste_to_image(img_aug_gt_dict, data_dict, points)

        # with open('/mnt/petrelfs/wuxiaopei/Visualize/Waymo/Semi-Sampling/%s.pkl' % data_dict['frame_id'],
        #           'wb') as f:
        #     pickle.dump(points, f)
        #     pickle.dump(gt_boxes, f)
        #     pickle.dump(gt_names, f)
        #     print('saved to /mnt/petrelfs/wuxiaopei/Visualize/Waymo/Semi-Sampling/%s.pkl' % data_dict['frame_id'])
        return data_dict

    def __call__(self, data_dict, use_db_sample):
        """
        这个函数是数据库采样器的主要调用函数。它从数据库中采样新的目标,并将它们添加到当前场景中。

        参数:
            data_dict: 包含当前场景信息的字典,其中包括:
                gt_boxes: (N, 7 + C) [x, y, z, dx, dy, dz, heading, ...] - 真实框信息
            use_db_sample: 是否使用数据库采样

        返回:
            data_dict: 添加了采样目标后的场景字典
        """
        # 获取当前场景中的真实框和类别名称
        gt_boxes = data_dict['gt_boxes']
        gt_names = data_dict['gt_names'].astype(str)
        existed_boxes = gt_boxes
        total_valid_sampled_dict = []  # 存储所有有效的采样目标
        sampled_mv_height = []  # 存储采样目标的高度调整值
        sampled_gt_boxes2d = []  # 存储采样目标的2D框

        # 对每个类别进行采样
        for class_name, sample_group in self.sample_groups.items():
            # 如果限制整个场景的目标数量,则根据当前场景中该类别的目标数量调整采样数量
        ...
        # 如果有有效的采样目标且允许使用数据库采样
        if total_valid_sampled_dict.__len__() > 0 and use_db_sample:
