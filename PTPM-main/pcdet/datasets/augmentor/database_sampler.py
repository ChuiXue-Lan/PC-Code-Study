...
class DataBaseSampler(object):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        if 'gt_score' in data_dict.keys():
            sampled_gt_score = np.ones(len(sampled_gt_boxes)).astype(np.float32)
            data_dict['gt_score'] = np.concatenate([data_dict['gt_score'], sampled_gt_score], axis=0)
