...
def transform_clouds(gt_points, src_box, tgt_box):
    gt_points = copy.deepcopy(gt_points)
    gt_points[:,0] -= src_box[0]
    gt_points[:,1] -= src_box[1]
    gt_points[:,2] -= src_box[2]

    theta = tgt_box[-1] - src_box[-1]
    gt_points = rotate_points_along_z_np(gt_points[None, :, :], np.array([theta]))[0]

    gt_points[:,0] += tgt_box[0]
    gt_points[:,1] += tgt_box[1]
    gt_points[:,2] += tgt_box[2]
