        ...
        for cur_epoch in tbar:
            ...
            if trained_epoch % ckpt_save_interval == 0 and rank == 0:
                ...
                if 's3://' in str(ckpt_save_dir):
                    ...
                    ckpt_list = petrel_client.list_dir_one_depth(ckpt_save_dir)
                    ...
                    epoch_list.sort()
                    if epoch_list.__len__() >= max_ckpt_save_num:
                        for cur_file_idx in range(1, trained_epoch - max_ckpt_save_num + 1):
                            petrel_client.remove(os.path.join(ckpt_save_dir, "checkpoint_epoch_%d.pth" % cur_file_idx))
...
def save_checkpoint(state, filename='checkpoint'):
    ...
    if False and 'optimizer_state' in state:
        ...
        if 's3://' in str(optimizer_filename):
            if torch.__version__ >= '1.4':
                petrel_client.save_ckpt(optimizer_filename, {'optimizer_state': optimizer_state}, _use_new_zipfile_serialization=False)
            else:
                petrel_client.save_ckpt(optimizer_filename, {'optimizer_state': optimizer_state})
        else:
            if torch.__version__ >= '1.4':
    ...
    if 's3://' in str(filename):
        if torch.__version__ >= '1.4':
            petrel_client.save_ckpt(filename, state, _use_new_zipfile_serialization=False)
        else:
            petrel_client.save_ckpt(filename, state)
    else:
        if torch.__version__ >= '1.4':
