    ...
    def forward(self, image, pc, text=None, Mask=None, val=False):
        ...
        if self.ulip:
            ...
            if hasattr(self, 'zeroshot_weights'):
                # 已调用 init_classifier，无需 text
                text_embed_all = self.zeroshot_weights  # [num_classes, dim]
            else:
                assert text is not None, "text 不能为空：未调用 init_classifier 时需要传入 text"
                text_embed_all = []
                for i in range(text.shape[0]):
                    text_for_one_sample = text[i]
                    text_embed = self.encode_text(text_for_one_sample)
...
def ULIP_MUST_PointBERT(args, classes, templates, tokenizer):
    ...
    if args.ulip:
        for k, p in model.named_parameters():
            ...
            elif 'pc_projection' in k:# or 'visual.mask_token_embedding' in k or 'visual.ln_patch' in k or 'visual.decoder' in k:
                p.requires_grad = True
            else:
                p.requires_grad = False

        # bertpretrain_ulip_model = torch.load('./checkpoints/pretrained_models_ckpt_zero-sho_classification_checkpoint_pointbert.pt',
        bertpretrain_ulip_model = torch.load('/home/cls2024/ltx/Replicate/ULIP-main/outputs/827/checkpoin-best.pt',
    ...
    if not args.from_scratch:
        ...
        # pretrained_ulip_model = torch.load('./checkpoints/merged_preview.pt',
                                           map_location=torch.device('cpu'))
