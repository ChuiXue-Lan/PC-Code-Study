...
# PartGeoZe类实现了一个基于几何特征的部件分割网络
class PartGeoZe(nn.Module):
    def __init__(self, sigma_d, sigma_a, sigma_e, angle_k, n_pts=128, voxel_size=0.05):
        super().__init__()
        # 初始化各种超参数
        self.sigma_d = sigma_d  # 距离相似度的sigma参数
        self.sigma_a = sigma_a  # 角度相似度的sigma参数 
        self.sigma_e = sigma_e  # 特征相似度的sigma参数
        self.angle_k = angle_k  # 角度特征的k近邻数
        self.n_pts = n_pts      # 下采样点数
        self.voxel_size = voxel_size  # 体素大小
        
        # 初始化网络模块
        self.gattn = FPFHGAttn2(0.001, 32)  # FPFH特征的全局注意力层
        self.sattn = FPFHLAttn(sigma_e, radius=0.15, n_sample=48)  # FPFH特征的局部注意力层
        self.s2pattn = S2PAttn(0.1, 0)  # 集合到点的注意力层
        self.dec_np = GeoDecoder(1, 8)   # 几何解码器
