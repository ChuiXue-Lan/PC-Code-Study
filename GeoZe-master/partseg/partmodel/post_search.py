...
seg_num = [4, 2, 2, 4, 4, 3, 3, 2, 2, 2, 6, 2, 3, 3, 3, 3] # 每个类别的分割部件数
...
def up_run_epoch(transformer, val_pc, val_normal, val_fpfh, val_feat, val_label, val_ifseen, val_pointloc, text_feat, part_num, class_choice,
    ...
    # 初始化参数
    val_size = val_feat.shape[0]  # 验证集大小
    bs = 15  # batch size
    iter = val_size // bs  # 迭代次数
    pred_seg, label_seg, class_label = [], [], []  # 存储预测结果、标签和类别
    num_node = 2048  # 每个点云的点数
