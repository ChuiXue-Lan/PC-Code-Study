...
class ShapeNet(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        picked_model_rendered_depth_addr = self.rendered_image_addr + '/' +\
                                           sample['taxonomy_id'] + '-' + sample['model_id'] + '/'
        picked_depth_name = sample['model_id'] + '_r_' +\
                            str(random.choice(self.picked_rotation_degrees)) + '_depth0001' +'.png'
        picked_depth_addr = picked_model_rendered_depth_addr + picked_depth_name

        try:
            image = pil_loader(picked_image_addr)
            depth = pil_loader(picked_depth_addr)
            image = self.train_transform(image)
            depth = self.train_transform(depth)
        except:
            raise ValueError("image is corrupted: {}".format(picked_image_addr))

        return sample['taxonomy_id'], sample['model_id'], tokenized_captions, data, image, depth
